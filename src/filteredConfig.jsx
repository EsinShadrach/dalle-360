			...
			<Disclosure as={`div`} className="w-full">
				{({ open }) => (
					<div className={`border w-full rounded-lg p-2 ${styling}`}>
						<Disclosure.Button as="div" className="flex items-center justify-between flex-wrap w-full">
                            {properties}
                            <ChevronDown
                                className={`${
                                ...
                                <Tab.Group vertical>
                                    <Tab.List className={`flex flex-col gap-2 w-full`}>
                                        {objectsOfEnvironment.map((environ)=>(
                                            <Tab key={environ} onClick={(e)=>sceneItemsFunc(e)} className="text-left p-2 rounded-md font-medium flex items-center justify-between hover:translate-x-2 duration-300 w-full">
                                                {({selected})=>(
